function [Out,all_remove,ICs] = detect_ICA_artefacts(S);
...
% Updated 22-5-17 to offer highpass filtering to 1Hz, which makes ICA more stable (at least for EEG) - hacked by assuming only one FiltPar passed, which is sampling rate
%
...
%
% Note - if applied to average-referenced EEG, should probably discard one
% channel to ensure data not rank-deficient.
%
% Fields of Out:
...
if length(FiltPars) == 3
    fprintf('Bandpass filtering from %d to %d Hz, with sampling rate %d Hz (warning - can fail)\n',FiltPars(1), FiltPars(2), FiltPars(3));
    for r=1:Nrefs(1)
...
elseif length(FiltPars) == 2
    fprintf('Lowpass filtering to %d Hz, with sampling rate %d Hz\n',FiltPars(1), FiltPars(2));
    for r=1:Nrefs(1)
...
elseif length(FiltPars) == 1
    fprintf('Highpass filtering to 1 Hz, with sampling rate %d Hz\n',FiltPars(1));
    for r=1:Nrefs(1)
        refs.tem{r} = ft_preproc_highpassfilter(refs.tem{r}, FiltPars(1), 1,  5, 'but','twopass','reduce');
    end
    ICs  = ft_preproc_highpassfilter(ICs,  FiltPars(1), 1,  5, 'but','twopass','reduce')';
else
